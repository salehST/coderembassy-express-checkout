...
(function($) {
    ...
    function initProductCheckboxes($container, quickCart) {
    ...
    function initProductClickHandlers($container) {
        // Remove any existing event handlers to prevent duplicates
        ...
        $container.find('.coderembassy-product-item').on('click.coderembassy', function(e) {
            // Don't trigger if clicking on checkbox/radio or its label
    ...
    function initVariationHandlers($container) {
        ...
        $container.find('.coderembassy-variation-option input[type="radio"]').on('change', function() {
            ...
            setTimeout(function() {
                ...
                if (needsRestore) {
                    // Re-trigger the change event after restoration
    ...
    function initAddToCartButton($container, ajaxCart) {
        // Remove any existing event handlers to prevent duplicates
                ...
                if (response.success) {
                    ...
                    if (response.data.cart_fragments && response.data.cart_fragments['div.widget_shopping_cart_content']) {
                        ...
                        if ($miniCart.length > 0) {
                            ...
                            // Fallback: try to find any element with the exact class
    ...
    function addToCartQuick($container, productIds, $productItem) {
    ...
    function addToCartForm($container, productIds) {
        // Create a form for non-AJAX submission
        ...
        $.each(productIds, function(index, productData) {
            if (typeof productData === 'object' && productData.product_id) {
                ...
                // Old format: just product ID number
    ...
    function clearSelectedProducts($container) {
    ...
    function showMessage($container, type, message) {
        // Use the new popup notification system
    ...
    function showNotification(type, message, duration) {
        ...
        var $notification = $('<div id="' + notificationId + '" class="' + notificationClass + '">' +
            '<div class="coderembassy-notification-content">' +
                '<div class="coderembassy-notification-icon">' +
                    '<span class="dashicons ' + iconClass + '"></span>' +
                '</div>' +
                '<div class="coderembassy-notification-message">' + message + '</div>' +
                '<div class="coderembassy-notification-close">' +
    ...
    function loadCheckoutForm($checkoutContent) {
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                $checkoutContent.html('<div class="coderembassy-checkout-form"><p class="coderembassy-checkout-error">' + 
                    'Failed to load checkout form. Please refresh the page.' + 
                    '</p></div>');
            }
        });
    }
